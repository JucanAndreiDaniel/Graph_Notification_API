...
try:
    start = time.time()
    import oldAPI.models as mod
    from oldAPI.models import market_chart
    ...
    for i in range(1, 26):
        ...
        for j in range(250):
            ...
            for k in range(4):
                ...
                try:
                    last_value = (
                        mod.value.objects.filter(currency=currencies[k])
                        .get(coin__coin_id=price_list[j]["id"])
                        .current
                    )
                except:
                    last_value = 0
                crypto_value = mod.value(
                    ...
                    ath_time=correcttime(coinFromList["ath_date"]),
                    atl=coinFromList["atl"],
                    atl_time=correcttime(coinFromList["atl_date"]),
                    percentage_1d=coinFromList["price_change_percentage_24h"],
                    percentage_1h=coinFromList[
