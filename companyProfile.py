...
try:
    ...
    for i in tickers:
        ...
        symbol = i['symbol']
        r = requests.get(
            f'https://finnhub.io/api/v1/stock/profile2?symbol={symbol}&token={token}')  # company info
        values = r.json()
        if not bool(values):
            continue
        print(values)
        companyProfile = CompanyProfile(country=values['country'] if values['country'] is not None else 'N/A',
                                        ...
                                        shareOutstanding=values['shareOutstanding'] if values[
                                            'shareOutstanding'] is not None else 0,
