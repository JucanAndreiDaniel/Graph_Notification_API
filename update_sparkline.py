...
try:
    ...
    for i in range(1, 26):
        ...
        for curr in currencies:
            ...
            price_list += price
        for j in range(250):
            cryptoobject= cryptoObject.objects.get(coin_id=price_list[j]["id"])
            for k in range(4):
                ...
                for l in range(1, 8):
                    try:
                        ...
                        crypto_market = market_chart(
                            coin_currency_day=f"{price_list[j]['id']}_{currencies[k]}_{(l-1)}",
                            coin=cryptoobject,
                            currency=currencies[k],
                            day=market_chart.Days(f"{l}"),
                            price1=prices[0],
                            price2=prices[1],
                            price3=prices[2],
                            price4=prices[3],
